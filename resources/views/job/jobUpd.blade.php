    ...
    <div style="font-size: 50px; margin:0;" >
        <form action="/jobUpddo">
            ...
            <div class="form-group">
                ...
                <select name="catelog_id" value="{{$data['catelog_id']}}" id="" class="form-control cate">
                   @foreach($catalog as $l)
                    <option value="{{$l['catelog_id']}}" @if($l['catelog_id']==$data['catelog_id']) selected @endif>{{$l['catelog_name']}}</option>
                       @endforeach
                </select>
            </div>
